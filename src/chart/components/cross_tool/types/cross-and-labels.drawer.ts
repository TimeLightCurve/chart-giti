...
export class CrossAndLabelsDrawerType implements CrossToolTypeDrawer {
	...
	protected drawCrossTool(ctx: CanvasRenderingContext2D, hover: CrossToolHover) {
		...
		if (allPanesHT(hover.x, allPanes.y)) {
			...
			ctx.setLineDash([]); // Clear dashed line for the circle

			const gradientRadius = 7; // Outer circle radius
			// const gradient = ctx.createLinearGradient(
			//     hover.x - gradientRadius, hover.y - gradientRadius,
			//     hover.x + gradientRadius, hover.y + gradientRadius
			// );
			// gradient.addColorStop(0, this.config.colors.areaTheme.circleStart);
			// gradient.addColorStop(1, this.config.colors.areaTheme.circleStop);

			// ctx.fillStyle = gradient;
			ctx.fillStyle = this.config.colors.crossTool.circleStop;
			ctx.strokeStyle = this.config.colors.crossTool.circleStart;
			ctx.lineWidth = 1; // Stroke width for the gradient circle
			ctx.beginPath();
			ctx.arc(hover.x, hover.y, gradientRadius, 0, 2 * Math.PI);
			ctx.fill();
			ctx.stroke();

			// Draw the inner solid circle
			const innerCircleRadius = 4; // Inner circle radius
			ctx.fillStyle = this.config.colors.crossTool.circleStop;
			ctx.strokeStyle = this.config.colors.crossTool.circleStart;
			ctx.lineWidth = 1;
			ctx.beginPath();
			ctx.arc(hover.x, hover.y, innerCircleRadius, 0, 2 * Math.PI);
			ctx.fill();
			ctx.stroke();

			ctx.restore(); // Restore previous state (including dashed lines)
